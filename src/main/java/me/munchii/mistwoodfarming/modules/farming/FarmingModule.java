    ...
    public void onEnable() {
        getCommandManager()
                .registerCommand(IglooCommand.create("help")
                        ...
                        .withUsage("/help")
                        .withAction((ctx) -> {
                            ctx.getPlayer().sendMessage(Chat.color("&8--- &aFarming Help &8---"));
                            getCommandManager().getCommands().forEach(command -> {
                                ctx.getPlayer().sendMessage(Chat.color(" &8- &a" + command.getUsage() + " &7" + command.getDescription()));
                            });
                            getCommandManager().getCommandGroups().forEach(group -> {
                                group.getSubCommands().forEach(command -> {
                                    ctx.getPlayer().sendMessage(Chat.color(" &8- &a/" + group.getGroupCommand() + " " + command.getUsage().replace("/", "") + " &7" + command.getDescription()));
                                });
                            });
                            return true;
                        })
